...
* [CTRLRun](https://github.com/CTRLRun/ctrlrun) - _Execution safety layer between an agent's decision to act and the call that acts. A YAML policy answers allow, ask a human, or deny for every consequential action before it leaves the process; approvals are single-use and bound to the hash of the exact action, one logical effect runs at most once across threads, processes and hosts, and an unknown outcome is recorded AMBIGUOUS rather than FAILED so a blind retry is refused until a human or a reconcile hook resolves it. Fails closed, carries authority grants with delegation limits and rolling budgets, and leaves a hash-chained JSON receipt for every attempt, refusals included. Python decorator, an MCP gateway for tools in any language, and LangGraph and OpenAI Agents adapters. Apache-2.0._
