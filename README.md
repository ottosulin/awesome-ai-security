...
* [node9](https://github.com/node9-ai/node9-proxy) - _Security and governance layer for AI coding agents and MCP servers (Claude Code, Codex CLI, Gemini CLI, Cursor, Copilot CLI and more) that checks every tool call before it runs: a credential jail, secret and PII detection, rules for destructive git, SQL and shell commands, per-service shields, human approval for held actions, MCP tool pinning, an egress allowlist and container sandbox, plus machine posture scoring, repo scanning for hijackable agent workflows, and one audit record across agents._
