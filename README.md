...
* [DOS Kernel](https://github.com/anthony-chaudhary/dos-kernel) - _Deterministic trust kernel for AI-agent fleets: verifies "done" claims against git evidence instead of agent self-report, arbitrates concurrent file access between agents, and audits commit messages against their own diffs. No LLM calls; hooks for Claude Code, Cursor, Codex, Gemini CLI plus an MCP server._
