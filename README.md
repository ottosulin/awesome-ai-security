...
* [SourceryKit](https://github.com/ProvablyAI/sourcerykit) - _Python SDK that verifies an AI agent's outbound requests against a source of truth using zero-knowledge proofs, so a call only goes out if the agent's claims check out. Hooks into the HTTP libraries to log every outbound call and block anything off the trusted-endpoint allow-list, MCP handoffs included._
