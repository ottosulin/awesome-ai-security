...
* [prompt-protection](https://github.com/mughalhere/prompt-protection) - _Zero-dependency TypeScript tool-call guard for JS agents. Labels tool results as untrusted and blocks flows from them into network, email, exec, file-write and payment sinks; also ships spotlighting, canary leak detection and an Agent Threat Rules (ATR) rule-pack loader. Published benchmark including its own false-positive rate._
...
* [prompt-protection agent-security datasets](https://huggingface.co/datasets/promptprotection/agent-security-datasets) - _CC-BY-4.0. 130 prompt injections written to evade pattern matching, 155 benign prompts carrying injection trigger vocabulary, and 100 agent tool-call flows labelled with the expected guard decision._
