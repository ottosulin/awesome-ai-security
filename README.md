...
* [jevc](https://github.com/doronp/jevc) - _Compiles agent guardrail prose into the deterministic verdict program a gate enforces. The instruction files a project already has (CLAUDE.md, AGENTS.md, skill files) are lowered to narrow typed evidence questions plus a first-match reducer in code, so the model supplies evidence and never returns the verdict; a lowered program that asks for a verdict is rejected by the linter. Capability-checked emitters for the Vercel AI SDK, LangChain and policy YAML refuse rather than silently weakening a rule the target grammar cannot express. Thresholds are measured, not assumed - of 58 predicted thresholds in the shipped corpus only 23 survived contact with the live model, and all 58 fixtures replay offline. TypeScript, Apache-2.0._
