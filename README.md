...
* [memory-blackbox](https://github.com/lavkumarv/memory-blackbox) - _Post-incident forensics for AI agent memory. Captures every memory read and write into an append-only, Ed25519-signed ledger (BLAKE3 hash chain, signed Merkle checkpoints) plus a provenance DAG, then traces a harmful action back to the memory that caused it, computes the forward closure of a poisoned source, and quarantines it by appending rather than deleting. Adapters for Mem0, Chroma, Letta, pgvector and `MEMORY.md`/`CLAUDE.md` files, plus an MCP gateway and a sidecar for hosted vector DBs. Reconstruction, not runtime prevention. Apache-2.0._
