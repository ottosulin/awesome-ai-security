...
* [forge-agent-gate](https://github.com/forgeorbital/forge-agent-gate) - _Local, deterministic authorization gate and verifiable proof trail for AI agents that take consequential actions (payments, refunds, approvals, procurement, trades). Ships as an MIT npm package and MCP server: checks each action against a signed mandate before it runs, blocks or escalates anything out of policy, and records a replayable proof trail. No LLM in the decision path._
