...
* [writ](https://github.com/writ-agent/writ) - _Authorization and provenance for AI agent tool calls: one policy file (allow / deny / ask / redact) checked before every call via Claude Code, Codex, Gemini CLI and Cursor hooks, Python/TypeScript SDKs and an MCP proxy; a kernel write boundary for launched agents; and a hash-chained, tamper-evident ledger with signed receipts._
