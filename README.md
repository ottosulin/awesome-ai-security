...
* [Nika](https://github.com/supernovae-st/nika) - _Intent-as-code workflow engine whose runs are constrained by construction: default-deny `permits:` for network/filesystem/secrets audited before execution, sandboxed exec, agent loops on explicit tool allowlists with turn/token budgets, and tamper-evident hash-chained run traces verifiable offline._
