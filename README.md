...
* [trapcheck](https://github.com/agentatwork/trapcheck) - _Pre-flight scanner that checks a GitHub repository for agent-targeting prompt injection before a coding agent is pointed at it. Reads the text an agent would read (issue bodies, CONTRIBUTING.md, AGENTS.md, CLAUDE.md, .cursorrules) and flags system-prompt exfiltration asks, directives hidden in HTML comments, and fake-bounty signals. Single file, stdlib only, exits non-zero on a trap so it can gate a pipeline._
