...
* [Agent evidence vectors](https://github.com/probityai/agent-evidence-vectors) - _Conformance corpus and reference verifier for the in-toto adversarial-execution-evidence predicate; a verifier is checked by replaying the corpus, and the outcome is recomputed from the bytes an attestation carries rather than read from a producer's verdict._
