...
* [gate.cat](https://github.com/BGMLAI/gate.cat) - _Deterministic, fail-closed deny-list that vetoes an AI coding agent's catastrophic shell commands (rm -rf /, disk wipe, fork bombs, secret exfil, curl-piped-to-shell) via pre-tool hooks, before they execute. Pip-installable, Apache-2.0. Pattern/rule-based rather than an LLM, so it covers a curated deny-list and can miss commands outside it; publishes its own documented false-block measurement in-repo._
