...
* [Skarn](https://getskarn.com/) - _Security scanner and real-time guard for AI coding-assistant sessions (Claude Code, Cursor, Codex CLI, Gemini CLI, Copilot). Scans on-disk session logs for leaked credentials and prompt-injection-to-exfiltration chains, and a pre-execution hook can block a leaked secret before the tool call runs. Local-first, no telemetry, SARIF output, 0-100 risk score. Commercial with a free tier._
