...
* [Agentmetry](https://github.com/blitzcrieg1/agentmetry) - _Local-first flight recorder for AI coding agents (Cursor, Claude Code, Codex, Antigravity, Qwen, Kimi, Qoder, CodeBuddy) and MCP servers via a stdio audit proxy. Hash-chained JSONL trail with RFC 6962 Merkle roots and inclusion proofs, 15 MITRE ATT&CK-mapped sequence detection rules with a public benchmark corpus, and a 300s coverage heartbeat reporting which agent surfaces are covered, uncovered, absent or unknown so a removed hook becomes visible rather than silent. Fingerprints what MCP servers return from tools/list over time to catch rug pulls where the config file is unchanged. Forwards to Splunk HEC, Elastic ECS, Google SecOps UDM and CloudEvents. Windows MSI and Intune deployment. Apache-2.0._
