...
* [Prismor / immunity-agent](https://github.com/PrismorSec/immunity-agent) - _Runtime security monitor for AI coding agents (Claude Code, Cursor, Windsurf). Hooks into PreToolUse/PostToolUse pipelines before commands reach the OS, evaluating them against YAML-configurable policies to block dangerous actions, secret exfiltration, privilege escalation, reverse shells, and prompt injection. Includes a secret cloaking layer, MCP/skill scanner, egress allowlist, and full session audit logs._
