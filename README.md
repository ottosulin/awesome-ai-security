...
* [RMT Platform](https://github.com/rajab2030/rmt-platform) - _Self-hosted governance layer for AI-agent actions. Every consequential action an agent proposes passes policy and risk checks, is held for human approval when required, and only then executes through a registered adapter, producing a correlated evidence chain (authorization, approval, audit, trace, verification). Evidence exports as HMAC-signed bundles verifiable offline with a stdlib-only script. MIT._
