...
* [Airlock](https://github.com/cyberbobas/airlock) - _Runtime firewall for AI coding agents that gates every tool, MCP, and skill call against a least-privilege policy, blocking secret-path reads, known-collector egress, and download-and-execute. TOFU toolset pinning detects MCP rug-pulls; hash-chained tamper-evident audit log; byte-for-byte uninstall. Gates Claude Code (native tools + MCP) plus Cursor, Windsurf, Cline, Kimi, grok, mimo and DeepSeek._
