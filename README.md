...
* [bounty-trap-scan](https://github.com/sujeito-operator/bounty-trap-scan) - _Scanner and snapshot of the public GitHub bounty issues that instruct the contributing agent to paste its own pre-conversation initialization payload into the file it is being paid to change — 91 of 563 open bounty-labelled issues (16.2%), most of them also labelled for agents only. Ships the measured compliance rate as well: across 1,200 pull requests in the three largest such repositories, 0 pasted a payload and 36 blocks published a real working directory (19 pull requests, 4 accounts). stdlib-only Python, raw JSON committed, MIT._
