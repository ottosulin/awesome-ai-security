...
* [Nemesis Forge](https://github.com/eobi/nemesisforge) - _Autonomous memory-safety discovery engine for C/C++. An LLM proposes harnesses and triage; eight deterministic oracles decide what is certified, so the whole verification path runs with no API key. Findings are reported at the rung of an exploitability ladder the evidence reaches, and each one states what was not established._
