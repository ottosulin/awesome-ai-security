...
* [gate.cat](https://github.com/BGMLAI/gate.cat) - _Deterministic, fail-closed deny-list that vetoes an AI coding agent's irreversible shell commands (rm -rf, disk wipe, DB drop, secret exfiltration, runaway loops) before they run — no model in the decision path, sub-millisecond. Installs as a native Claude Code / Cursor hook or a gated shell for Codex/aider. Apache-2.0. Catches known dangerous command shapes; obfuscated or novel commands can still slip, and the bypass suite is published in-repo._
