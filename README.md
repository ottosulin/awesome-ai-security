...
* [AgentSight](https://github.com/eunomia-bpf/AgentSight) - _Zero-instrumentation eBPF observability for LLM/coding agents. Captures syscall-level traces (process, file, network) without modifying the agent._
* [ActPlane](https://github.com/eunomia-bpf/ActPlane) - _OS-level agent harness that compiles a policy DSL to an eBPF engine for labeled information-flow control at the syscall boundary, with corrective feedback._
