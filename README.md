...
* [sofagent](https://github.com/KongFangXun/sofagent) - _Commit-time audit engine for AI coding agents (Claude Code, Codex, etc.) — scans git diffs with 24 audit rules (secret leaks, out-of-scope edits, blind modifications without prior reads, prompt injection in commit messages, unauthorized file changes), writes an HMAC-signed tamper-evident local audit history, and ships MCP tools for governance aggregation. MIT._
